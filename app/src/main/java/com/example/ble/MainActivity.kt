...
class MainActivity : AppCompatActivity() {
    ...
    private val scanResultAdapter: ScanResultAdapter by lazy {
        ...
        ScanResultAdapter(scanResults) { result ->
            ...
            with(result.device) {
                ...
                connectGatt(this@MainActivity, false, gattCallback) //CONTEXT????
    ...
    fun startBleScan() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && !isLocationPermissionGranted) {
            ...
            bleScanner.startScan(mutableListOf(scanFilter), scanSettings, scanCallback) //optionally add scan filtes!!
            isScanning = true
        }
    }
    //stop scanning
